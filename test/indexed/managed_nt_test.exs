defmodule Indexed.ManagedNamedTableTest do
  @moduledoc """
  Test managed but with a named table.
  """
  ...
  defp record(name, id, preload), do: BlogServerNT.run(& &1.get.(name, id, preload))
  defp records(name), do: BlogServerNT.run(& &1.get_records.(name))
  defp paginate, do: BlogServerNT.paginate(preload: preload())
  defp entries, do: paginate().entries

  defp basic_setup do
    {:ok, %{id: bob_id} = bob} = BlogNT.create_user("bob", ["pin"])
    {:ok, %{id: jill_id}} = BlogNT.create_user("jill", ["hat", "mitten"])
    {:ok, %{id: lee_id}} = BlogNT.create_user("lee", ["wig"])
    {:ok, %{id: lucy_id}} = BlogNT.create_user("lucy")
    {:ok, _} = BlogNT.update_user(bob, %{best_friend_id: lucy_id})
    ...
    bs = start_supervised!(BlogServerNT.child_spec(feedback_pid: self()))
  ...
  test "a runthrough / scenario" do
    ...
    bob = BlogNT.get_user("bob")
    {:ok, _} = BlogNT.update_user(bob, %{name: "fred"})
    ...
    {:ok, _} = BlogNT.delete_comment(comment_id)
  ...
  test "update entity which has foreign one AND many connections" do
    ...
    {:ok, _} = BlogNT.update_comment(comment_id, msg)

    assert %{content: ^msg} = BlogServerNT.get_comment(comment_id)
    assert %{content: ^msg} = entries() |> hd() |> Map.fetch!(:comments) |> hd()
  end
  ...
  test "Nesty McNesterson" do
    ...
    {:ok, %{id: mar_id}} = BlogNT.create_user("mar")
    {:ok, _} = BlogNT.update_user(bob, %{best_friend_id: lucy_id})
    {:ok, _} = BlogNT.update_user(lucy, %{best_friend_id: mar_id})

    [%{id: post_id, content: "Ya"}] = entries()
    {:ok, _} = BlogNT.update_post(post_id, content: "hey")
